...
generated quantities {
  ...
  // Posterior and Prior predictive ratings
  array[n_populations, n_agents, n_stim] int post_pred_rating;
  array[n_populations, n_agents, n_stim] int prior_pred_rating;
  ...
  for (p in 1:n_populations){
    ...
    for (i in 1:n_agents){
      ...
      for (j in 1:n_stim){
        ...
        // Prior predictive (not conditioned on data)
        prior_pred_rating[p, i, j] = beta_binomial_rng(7, alpha_post, beta_post);

        // Posterior predictive (after conditioning on data)
...
```{r}
colnames(draws_df_weighted)

# Extract Prior Predictions
prior_predictions_weighted <- draws_df_weighted %>%
  select(starts_with("prior_pred_rating[")) %>%
  pivot_longer(cols = everything(),
               names_to = "index",
               values_to = "prediction") %>%
  mutate(
    index = str_extract(index, "\\[(\\d+),(\\d+),(\\d+)\\]"),
    population = as.integer(str_match(index, "\\[(\\d+),")[,2]),
    agent = as.integer(str_match(index, "\\[(\\d+),(\\d+),")[,3]),
    trial = as.integer(str_match(index, "\\[(\\d+),(\\d+),(\\d+)\\]")[,4])
  ) %>%
  select(population, agent, trial, prediction)

# Summarise Prior Predictions
prior_summary_weighted <- prior_predictions_weighted %>%
  group_by(trial) %>%
  summarise(
    mean = mean(prediction),
    lower = quantile(prediction, 0.025),
...
posterior_predictions_weighted <- draws_df_weighted %>%
  select(starts_with("post_pred_rating[")) %>%
  pivot_longer(cols = everything(),
               names_to = "index",
               values_to = "prediction") %>%
...
posterior_summary_weighted <- posterior_predictions_weighted %>%
  group_by(trial) %>%
  summarise(
    mean = mean(prediction),
    lower = quantile(prediction, 0.025),
...
# Convert true second_rating data to tidy format
true_rating_weighted <- as_tibble(data_weighted$second_rating[1,,]) %>%
  mutate(agent = row_number()) %>%
  pivot_longer(cols = -agent, names_to = "trial", values_to = "rating") %>%
  mutate(trial = as.integer(str_extract(trial, "\\d+")))
...
# Plot: Prior Predictive Check
p1_weighted <- ggplot() +
  geom_ribbon(data = prior_summary_weighted, aes(x = trial, ymin = lower, ymax = upper), 
              alpha = 0.2, fill = "red") +
  geom_line(data = prior_summary_weighted, aes(x = trial, y = mean), color = "red") +
  geom_point(data = true_rating_weighted, aes(x = trial, y = rating), alpha = 0.2) +
  labs(title = "Prior Predictive Check (Weighted Model)", x = "Trial", y = "Rating") +
  theme_minimal()

# Plot: Posterior Predictive Check
p2_weighted <- ggplot() +
  geom_ribbon(data = posterior_summary_weighted, aes(x = trial, ymin = lower, ymax = upper),
              alpha = 0.2, fill = "blue") +
  geom_line(data = posterior_summary_weighted, aes(x = trial, y = mean), color = "blue") +
  geom_point(data = true_rating_weighted, aes(x = trial, y = rating), alpha = 0.2) +
  labs(title = "Posterior Predictive Check (Weighted Model)", x = "Trial", y = "Rating") +
  theme_minimal()

# Display Plots
p1_weighted + p2_weighted
...
```{r}
#I MADE A CHANGE SO IT RUNS ON MY LAPTOP
# Load data
data <- read_csv("/Users/cassandrarempel/Desktop/Cognitive Science MSc/Advanced Cognitive Modelling/Simonsen_clean.csv", show_col_types = FALSE,
                 col_select = c("ID", "FaceID", "FirstRating", "GroupRating", "SecondRating"))
...
model_weights <- loo::loo_model_weights(
  list("Simple Integration" = loo_simple,
       "Weighted Integration" = loo_weighted)
)
