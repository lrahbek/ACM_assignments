...
```{r}
emp_data_sum <- pivot_wider(emp_data[c("stimulus", "correct", "dangerous", "cycle")], 
                            names_from = correct, values_from = correct, values_fn = length, 
                            values_fill = 0)
colnames(emp_data_sum) <- c("stimuli", "danger", "cycle", "wrong", "correct")
emp_data_sum$stimuli <- str_split_fixed(emp_data_sum$stimuli, pattern = ".jpg", n = 2)[,1]
...
```{r}
experiment <- function(agent_id, w, c, stimuli, cycles, danger_fun = "ADD", 
                       danger_features, danger_vals){
  trials <- length(stimuli)*cycles
  n_stim <- length(stimuli)
  stim_df <- data.frame(w = rep(w, trials), c = rep(c, trials), 
...
gcm_agent <- function(w, c, obs, cat_list){
  ...
  for (i in 1:ntrials){
    ...
    if (i == 1 || sum(cat_list[1:(i - 1)]) == 0 || sum(cat_list[1:(i - 1)]) == (i - 1)) {
      ...
      denominator <- mean(similarities[cat_list[1:(i - 1)] == 1]) + mean(similarities[cat_list[1:(i - 1)] == 0])
...
for (j in 1:length(w_ls)){
  ...
  for(i in 1:n_agents){
    ...
    exp_df$a_choices <- gcm_agent(w = w, c = c, 
                                  obs = exp_df[c("eyes", "legs", "spots", "arms", "color")], 
