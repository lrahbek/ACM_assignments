...
# Format for plotting
plot_df_c <- draws_c_all %>%
  select(value, c_prior) %>%
  pivot_longer(cols = everything(), names_to = "source", values_to = "value") %>%
  mutate(source = recode(source, value = "Posterior", c_prior = "Prior"),
         source = factor(source, levels = c("Prior", "Posterior")))  # FORCE factor levels

# Plot with legend
ggplot(plot_df_c, aes(x = value, fill = source)) +
  geom_histogram(alpha = 0.4, position = "identity", bins = 30) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black") +
  labs(title = "Posterior vs Prior (All agents): Sensitivity Parameter",
       x = "c", y = "Count", fill = "Distribution") +  # Optional: set legend title
  scale_fill_manual(values = c("Posterior" = "lightblue", "Prior" = "pink")) +
  theme_bw() +
  theme(
    legend.position = "right",
    legend.title = element_blank(),  # or set your preferred title above
    legend.text = element_text(size = 10)
  )

#Long format dataframe
plot_df_w1 <- draws_w1_all %>%
  select(value, w_prior) %>%
  pivot_longer(cols = everything(), names_to = "source", values_to = "value") %>%
  mutate(
    source = recode(source, value = "Posterior", w_prior = "Prior"),
    source = factor(source, levels = c("Prior", "Posterior"))
  )

ggplot(plot_df_w1, aes(x = value, fill = source)) +
  geom_histogram(alpha = 0.4, position = "identity", bins = 30) +
